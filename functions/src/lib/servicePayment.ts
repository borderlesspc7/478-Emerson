...
export async function loadCatalogService(serviceId: string) {
  const db = adminDb()
...
export async function promotePaymentToServiceRequest(
  paymentId: string,
  chargeHint: { orderId?: string | null; chargeId?: string | null } = {},
) {
  const db = adminDb()
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (payment.paymentStatus === 'paid' && payment.serviceRequestId) {
      return {
        ok: true as const,
        serviceRequestId: payment.serviceRequestId as string,
        already: true,
      }
    }

    const requestRef = payment.serviceRequestId
      ? db.collection(SERVICE_REQUESTS_COLLECTION).doc(payment.serviceRequestId as string)
    ...
    return { ok: true as const, serviceRequestId: requestRef.id, already: false }
  })
}

export function shouldMarkPaidFromCharge(chargeStatus: unknown, transactionStatus: unknown) {
